...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Animate Hero's elements
  const hero = document.getElementById("hero");
  let heroTl;
  function animateHero() {
    if (heroTl) return;
    gsap.to(".animated_cursor", {
      opacity: 0,
      duration: 0.4,
      repeat: -1,
      yoyo: true,
    ...
    heroTl = gsap.timeline({ resumed:true, repeat: -1 });
    heroWords.forEach((word) => {
      let heroTextTl = gsap.timeline({ repeat: 1, yoyo: true, repeatDelay: 2.5 });
      heroTextTl.to(".animated_words", {
        duration: 2.5,
        text: word,
        ease: "power2.inOut"
      });
      heroTextTl.to(".lower_phrase", {
        opacity: 1,
        y: "55%",
        duration: 0.5,
        ease: "none"
      });
      heroTl.add(heroTextTl);
    });
  }

  // Animate About section's grid images
  ...
  const aboutGrid = document.getElementsByClassName("about_grid")[0];
  let aboutGridTl;
  function animateAboutGrid() {
    if (aboutGridTl) return;
    aboutGridTl = gsap.timeline({ repeat: -1 });
    let gridImagesTl = gsap.timeline({ repeat: 1, yoyo: true, repeatDelay: 2 });
    gridImagesTl.to("#about_rect1", 0.5, {
      morphSVG: {shape: customPath1, shapeIndex: [-12], type: "linear", precompile: path1_preCompiled},
      delay: 2, ease: "Power1.inOut"}, -1);
    gridImagesTl.to("#about_rect2", 0.5, {
      morphSVG: {shape: customPath2, shapeIndex: [-2], type: "linear", precompile: path2_preCompiled},
      delay: 2, ease: "Power1.inOut"});
    gridImagesTl.to("#about_rect3", 0.5, {
      morphSVG: {shape: customPath3, shapeIndex: [18], type: "linear", precompile: path3_preCompiled},
      delay: 2, ease: "Power1.inOut"});
    aboutGridTl.add(gridImagesTl);
  }
  
  const animObserveOptions = {root: null, rootMargin: "0px", threshold: 0.5};
  ...
  animObserver.observe(hero);
  animObserver.observe(aboutGrid);
